...
then broadcasts a selected-scenario snapshot. Its scenario ID switches clients to the host's order of battle automatically. Scenarios 1–3 are public-information learning games; Scenarios 4–9 omit undetected opposing positions, units, paths, commands, contacts, logs, and rules transactions. Scenario 5 also redacts opposing dummy-card counts while publishing required transfer verification, and Scenarios 6–9 withhold the unit card for a located-but-unclassified submarine contact. Scenario 7 deployment, Scenario 8 edge entry/exit, and Scenario 9 aircraft and submarine-breakout commands are validated authoritatively by the host. Scenario 9 snapshots include serviceable aircraft, mission state, ready turn, once-per-turn attack use, and per-station sensor use.
