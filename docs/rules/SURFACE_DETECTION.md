...
Side-private projections expose friendly formations completely and classified enemy contacts completely. Undetected formations expose neither current position nor contents; located submarine/dummy contacts expose a search position but no card; lost contacts retain only their last-known position. Scenarios 4–7 additionally redact opposing commands, paths, contacts, logs, and rules transactions from network snapshots. Scenario 5 publishes required dummy-card transfer verification but never the opponent's remaining dummy count. Cyan pulsing rings indicate radiating radar; amber rings mark known enemy contacts. The authoritative debug trace is sealed during a hidden-information match and becomes available at game end.

Scenarios 4–7 use the redacted authoritative multiplayer snapshot schema, so hidden contacts, dummy deception, and undersea contacts are supported over direct IP and Relay. Scenarios 1–3 remain fully public because those learning scenarios omit detection.
